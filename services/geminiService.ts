...
export const getJargonGlossary = (documentText: string, language: Language) => {
    const targetLanguage = languageMap[language] || 'English';
    const prompt = `From the following legal document, identify up to 15 of the most important or confusing technical or legal jargon terms. For each term, provide a very simple, one-sentence definition that a non-expert can understand. Respond in ${targetLanguage}.\n\nDOCUMENT TEXT:\n---\n${documentText}\n---`;
