...
import { updateChatHistory } from '../services/firestoreService';
...
export const Chat: React.FC<ChatProps> = ({ isOpen, onClose, documentText, analysisResult, chatHistory, setChatHistory, currentAnalysisId }) => {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        const newHistory: Message[] = [...chatHistory, userMessage];
        setChatHistory(newHistory);
        setUserInput('');
        setIsThinking(true);

        try {
            const modelResponseText = await getChatResponse(documentText, analysisResult, newHistory, trimmedInput, language);
            const modelMessage: Message = { role: 'model', text: modelResponseText };
            setChatHistory(prev => [...prev, modelMessage]);
            if (currentUser && currentAnalysisId) {
                await updateChatHistory(currentUser.uid, currentAnalysisId, [userMessage, modelMessage]);
            }
        } catch (error) {
            console.error("Chat error:", error);
            const errorMessage = (error as Error).message || "Sorry, I couldn't get a response. Please try again.";
            const errorModelMessage: Message = { role: 'model', text: `Error: ${errorMessage}` };
            setChatHistory(prev => [...prev, errorModelMessage]);
            if (currentUser && currentAnalysisId) {
                await updateChatHistory(currentUser.uid, currentAnalysisId, [userMessage, errorModelMessage]);
            }
        } finally {
            setIsThinking(false);
        }
