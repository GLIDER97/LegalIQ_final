...
const App: React.FC = () => {
  ...
  const handleAnalyze = useCallback(async (retrySection?: keyof AnalysisResult) => {
    ...
    setIsLoading(true);
    setError(null);
    let tempAnalysisId = currentAnalysisId;
    ...
    if (retrySection) {
        ...
        setCurrentAnalysisId(null);
        tempAnalysisId = null; // Ensure temp ID is also reset
        setSectionErrors([]);
    ...
    try {
        ...
        // Sanitize the result to remove any 'undefined' values before state update or DB save
        const sanitizedNewResult = JSON.parse(JSON.stringify(newAnalysisResult));
        const partialResult = {...analysisResult, ...sanitizedNewResult} as AnalysisResult;

        setAnalysisResult(partialResult);
        setSectionErrors(prev => [...prev.filter(e => !taskMap.includes(e.section)), ...newSectionErrors]);
        
        // --- Save to Firestore if this is a new, full analysis ---
        if (!retrySection && currentUser && newSectionErrors.length === 0) {
            try {
                const docRef = await db.collection('users').doc(currentUser.uid).collection('analyses').add({
                    fileName,
                    documentTitle: partialResult.documentTitle,
                    createdAt: firebase.firestore.FieldValue.serverTimestamp(),
                    analysisResult: partialResult, // Save the partial, sanitized data
                    chatHistory: [],
                });
                setCurrentAnalysisId(docRef.id);
                tempAnalysisId = docRef.id; // Store ID in a temp variable for this scope
            } catch (dbError: any) {
                console.error("Error saving initial analysis to Firestore:", dbError);
                console.error("Error code:", dbError?.code);
                console.error("Error message:", dbError?.message);
                console.error("Current user:", currentUser?.uid);
                console.error("Attempted path:", `users/${currentUser?.uid}/analyses/`);
                // Non-critical, but we can log it.
            }
        }
        // --- End of Firestore saving ---


        // Jargon glossary runs after everything else, using the original document text
        if (!retrySection && documentText.trim()) {
            setIsGlossaryLoading(true);
            try {
                const glossaryResult = await getJargonGlossary(documentText, language);
                setAnalysisResult(prev => {
                  if (!prev) return null;
                  const updatedResult = { ...prev, ...glossaryResult } as AnalysisResult;
                  
                  // Update Firestore with the glossary, using the ID we just got
                  if (tempAnalysisId && currentUser) {
                      db.collection('users').doc(currentUser.uid).collection('analyses').doc(tempAnalysisId)
                        .update({ 'analysisResult.jargonGlossary': glossaryResult.jargonGlossary })
                        .catch(e => console.error("Failed to update glossary in Firestore:", e));
                  }
...
}, [documentText, language, t, analysisResult, currentUser, fileName, currentAnalysisId]);
